...
{
	g_pDev_I2C_SW->SendByte(g_pDev_I2C_SW, Byte);
}
...
void OLED_WriteData(uint8_t* Data, uint8_t Count)
{
	...
	for (uint8_t i = 0; i < Count; i++) //循环Count次，进行连续的数据写入
	{
		//依次发送Data的每一个数据
...
{
	OLED_ShowBinNum(--Column * 8, --Line * 16, Number, Length, OLED_8X16);
	OLED_Update();
}
void OLED_ShowFloatNum_Easy(uint8_t Line, uint8_t Column, double Number, uint8_t IntLength, uint8_t FraLengthe)
{
	OLED_ShowFloatNum(--Column * 8, --Line * 16, Number, IntLength, FraLengthe, OLED_8X16);
	OLED_Update();
}
void OLED_ShowChinese_Easy(uint8_t Line, uint8_t Column, char* Chinese)
{
	OLED_ShowChinese(--Column * 16, --Line * 16, Chinese);
	OLED_Update();
}
void OLED_Printf_Easy(uint8_t Line, uint8_t Column, char* format, ...)
{
	OLED_Printf(--Column * 8, --Line * 16, OLED_8X16, format);
	OLED_Update();
}
